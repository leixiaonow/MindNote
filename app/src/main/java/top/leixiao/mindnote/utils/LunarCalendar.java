...
public class LunarCalendar {
    ...
    public static int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        ...
        for (i = 1; i < month; i++) {
            if ((LUNAR_INFO[year - 1900] & (0x00080000 >> (i - 1))) == 0) {
                dayOffset += 29;
            } else {
                dayOffset += 30;
            }
        ...
        if (leapMonth != 0 && (month > leapMonth || (month == leapMonth && isLeapMonth))) {
            dayOffset = (LUNAR_INFO[year + -1900] & (0x00080000 >> (month + -1))) == 0 ? dayOffset + 29 : dayOffset + 30;
        }
    ...
    private static int daysInLunarMonth(int year, int month) {
        if ((LUNAR_INFO[year - 1900] & (0x00100000 >> month)) == 0) {
            return 29;
        }
        return 30;
    }
    ...
    public static int[] parseLunarDate(String date) {
        int[] lunarDate = new int[3];
        if (validateDateFormat(date)) {
            String[] strings = date.split(DATE_SEPARATOR);
            lunarDate[0] = Integer.valueOf(strings[0]);
            lunarDate[1] = Integer.valueOf(strings[1]);
            lunarDate[2] = Integer.valueOf(strings[2]);
            return lunarDate;
        }
