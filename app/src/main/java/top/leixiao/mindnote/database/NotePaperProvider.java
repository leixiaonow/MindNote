...
import top.leixiao.mindnote.database.NotePaper.AccountConstract;
...
public class NotePaperProvider extends ContentProvider {
    ...
    private static HashMap<String, String> sLiveFolderProjectionMap=new HashMap<>();;
        ...
        switch (sUriMatcher.match(uri)) {
            case NOTES /*1*/:
                ...
                if (!values.containsKey(Notes.DESKTOP)) {
                    values.put(Notes.DESKTOP, FILE_COLUMN_ID);
                }
                if (TagData.FUN_ENCRYPT) {
                    if (!values.containsKey(Notes.ENCRYPT)) {
                        values.put(Notes.ENCRYPT, FILE_COLUMN_ID);
                    } else if (values.getAsInteger(Notes.ENCRYPT) == NOTES) {
...
//                        }
            ...
            case FILES /*11*/:
                if (!initialValues.containsKey(NoteFiles.DIRTY)) {
                    initialValues.put(NoteFiles.DIRTY, true);
                }
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (type) {
            case NOTES /*1*/:
                if (TagData.FUN_ENCRYPT && values.containsKey(Notes.ENCRYPT) && values.getAsInteger(Notes.ENCRYPT) == NOTES) {
...
//                    }
            ...
            case FILE_ID /*12*/:
                if (!values.containsKey(NoteFiles.DIRTY)) {
                    values.put(NoteFiles.DIRTY, true);
                }
            ...
            case TAG_ID /*14*/:
                if (!values.containsKey(NoteCategory.DIRTY)) {
                    values.put(NoteCategory.DIRTY, true);
                }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion <= NotePaperProvider.FILES) {
                ...
                try {
                    ...
                    ContentValues cv = new ContentValues();
                    cv.put(Notes.ENCRYPT, NotePaperProvider.FILE_COLUMN_ID);
