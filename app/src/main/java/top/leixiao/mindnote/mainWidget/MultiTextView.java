...
public class MultiTextView extends TextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setAlpha(TransportMediator.FLAG_KEY_MEDIA_NEXT);
        ...
        while (i < lineCount) {
            ...
            if (nextLine >= lineCount || nextLineBottom <= height) {
                ...
                canvas.drawText(TextUtils.ellipsize(TextUtils.substring(text, lineStart, text.length()), (TextPaint) paint, width, TruncateAt.END).toString(), (float) paddingLeft, (float) baseline, paint);
                return;
            }
        }
    }
